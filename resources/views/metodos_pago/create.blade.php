<x-dashboard-layout>
    <x-slot name="header">
        <h1 class="text-3xl font-bold">Agregar Método de Pago</h1>
    </x-slot>

    <form action="{{ route('metodos_pago.store') }}" method="POST" class="space-y-4 mt-4">
        @csrf
        ...
        <div>
            ...
            <select id="nombre_metodo" name="nombre_metodo" class="w-full border rounded p-2" required onchange="toggleFields()">
                ...
                <option value="depositos en tiendas de servicios">Depósitos en Tiendas de Servicios</option>
            </select>
        </div>

        <div id="banco_field">
            <label for="banco" id="banco_label" class="block font-bold">Banco:</label>
            <input type="text" id="banco" name="banco" value="{{ old('banco') }}" class="w-full border rounded p-2">
        </div>

        <div id="propietario_cuenta_field">
            <label for="propietario_cuenta" class="block font-bold">Propietario de la Cuenta:</label>
            <input type="text" id="propietario_cuenta" name="propietario_cuenta" value="{{ old('propietario_cuenta') }}" class="w-full border rounded p-2">
        </div>

        <div id="numero_tarjeta_field">
            <label for="numero_tarjeta" class="block font-bold">Número de Tarjeta:</label>
            <input type="text" id="numero_tarjeta" name="numero_tarjeta" value="{{ old('numero_tarjeta') }}" class="w-full border rounded p-2">
        </div>

        <div id="clabe_field">
            <label for="clabe" class="block font-bold">CLABE:</label>
            <input type="text" id="clabe" name="clabe" value="{{ old('clabe') }}" class="w-full border rounded p-2">
        </div>

        <div>
            <label for="detalles" class="block font-bold">Detalles:</label>
            <textarea id="detalles" name="detalles" class="w-full border rounded p-2">{{ old('detalles') }}</textarea>
        </div>
